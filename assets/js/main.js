...
function(a) {
    ...
    c.onTagsLoaded = function(a) {
            if (!this._destroyed && (localStorage.setItem("lastUpdateCheck", Date.now()), a && Array.isArray(a) && 0 !== a.length)) {
                var b, c, d, e = require("semver"),
                    f = a.length;
                for (b = 0; b < f; b++)
                    if (c = a[b], d = c.tag_name, !c.prerelease && e.valid(d) && e.gt(d, this.currentTag)) {
...
function(a, b) {
    ...
    d._handleErrors = function(a) {
        ...
        d._onClose = function() {
            ...
            this.resizable && (c.width = a.width, c.height = a.height),
                localStorage.setItem("windowSettings", JSON.stringify(c)),
                a.hide(),
                this.browser && (this.browser.destroy(), this.browser = null),
                this.updater && (this.updater.destroy(), this.updater = null),
                this.close(),
...
function($) {
    ...
    p.changed = function() {
        ...
        p.init = function() {
            var a = this,
                b = (springroll.Application.instance, this.changed.bind(this));
            $("[data-toggle='tooltip']").tooltip({
                ...
                this.configDialog.on("show.bs.modal",
                    function() {
                        a.defaultConfigSelector.find("option:contains('-默认示例-')").prop("selected", !0),
                            a.configUpload.wrap("<form>").parent("form").trigger("reset"),
                            a.configUpload.unwrap(),
                            a.configPaste.val("")
                    }),
                this.imageDialog.on("show.bs.modal",
                    function() {
                        a.defaultImageSelector.find("option:contains('-默认图片-')").prop("selected", !0),
                            a.imageUpload.wrap("<form>").parent("form").trigger("reset"),
                            a.imageUpload.unwrap()
                    }),
                this.renderer.find("input").on("change",
                    function() {
        ...
        p.set = function(a) {
            ...
            if (a.blendMode && PIXI.particles.ParticleUtils.getBlendMode(a.blendMode))
                for (b = a.blendMode.toLowerCase(); b.indexOf(" ") >= 0;) b = b.replace("_");
            else b = "normal";
        ...
        p.get = function() {
            ...
            return "rect" == spawnType ? output.spawnRect = {
                ...
                } : "burst" == spawnType && (output.particlesPerWave = parseInt(this.emitParticlesPerWave.val()) || 1, output.particleSpacing = parseFloat(this.emitParticleSpacing.val()) || 0, output.angleStart = parseFloat(this.emitAngleStart.val()) || 0),
                output
        },
        ...
        p.download = function() {
            ...
            for (let i = 0; i < imageList.children.length; i++) {
                let particleImage = imageList.children[i].getElementsByClassName("actualImage")[0];
                base64Arr.push(getBase64(particleImage))
            }
            console.log("下载")
            let json = { urls: [], base64Arr: base64Arr, emitterConfig: this.get() }
                // var a = "ht.ParticlesClass(\"粒子\"," + JSON.stringify(this.get(), null, "\t") + "\n,[]);",
            ...
            c ? window.saveAs(new Blob([a], {
                type: b
            }), "emitter.particle") : window.open(encodeURI(b + "," + a))
        },
...
function() {
    ...
    p.onResize = function(a, b) {
        ...
        p.onInitialized = function() {
            ...
            for (var a, b = [], c = [], d = 0; d < this.config.emitters.length; d++) a = this.config.emitters[d],
                b.push({
                    id: a.id,
                    src: a.config,
                    complete: this.onConfigLoaded
                });
            for (var e in this.config.images) c.push(this.config.images[e]),
                b.push({
                    id: e,
                    type: "pixi",
            ...
            if (f)
                for (d = 0; d < f.length; d++) 0 !== f[d].indexOf("data:") && c.indexOf(f[d]) == -1 && (c.push(f[d]), b.push({
                    id: f[d],
                    type: "pixi",
                    image: f[d],
                    cache: !0
        ...
        p._onCompletedLoad = function(a) {
            ...
            for (e = 0; e < this.config.emitters.length; e++) {
                ...
                for (var g = 0; g < b.images.length; g++) c = b.images[g],
            ...
            if (i) this.loadDefault(i);
            else if (h && f)
                for (this.loadSettings(getTexturesFromUrls(f), h), this.setConfig(h), e = 0; e < f.length; ++e) this.addImage(f[e]);
            else this.loadDefault(this.config["default"]);
            this.on({
        ...
        p.setRenderer = function(a) {
            if ("webgl" != a || this.webgl) {
                ...
                var c = this[a];
                interaction && (interaction.off("stageup", this.onMouseUp), interaction.off("stagein", this.onMouseIn), interaction.off("stageout", this.onMouseOut), interaction.off("stagemove", this.onMouseMove)),
                    stage = c.stage,
    ...
    p.loadConfig = function(event) {
            var ui = this.ui,
                type, value, success = !1;
            if ("-默认示例-" != ui.defaultConfigSelector.val() ? type = "default" : ui.configPaste.val() ? type = "paste" : ui.configUpload[0].files.length > 0 && (type = "upload"), "default" == type) value = ui.defaultConfigSelector.val(),
                "-默认示例-" != value && (success = !0, this.loadDefault(value));
            else if ("paste" == type) {
                value = ui.configPaste.val();
                try {
                    ht.eval("var obj = " + elem.val() + ";"),
                        success = !0,
                        this.setConfig(obj),
                        this.loadFromUI()
                } catch (e) {}
                ...
                var files = ui.configUpload[0].files,
                    ...
                    onloadend = function(readerObj) {
                        let particle = JSON.parse(readerObj.result);
                        console.log("加载：", particle);
                        try {
                            // ht.eval("var obj = " + particle.emitterConfig + ";");
                            var obj = particle.emitterConfig;
                            console.log(obj);
                            scope.setConfig(obj);
                            this.removeAllImage();
                            for (let i = 0; i < particle.base64Arr.length; i++) {
                                this.addImage(particle.base64Arr[i]);
                            }
                            scope.loadFromUI()
        ...
        p.loadImage = function(a) {
            var b, c, d = this.ui,
                e = !1;
            // alert("heheh")
            console.log(d.defaultImageSelector.val(), d.imageUpload[0].files.length, a.result);
            if ("-默认图片-" != d.defaultImageSelector.val() ? b = "select" : d.imageUpload[0].files.length > 0 && (b = "upload"), "select" == b) c = d.defaultImageSelector.val(),
                "-默认图片-" != c && (e = !0, this.addImage(c), this.loadFromUI());
            else if ("upload" == b) {
    ...
    p.loadFromUI = function() {
        ...
        p.update = function(a) {
            emitter && (emitter.update(.001 * a), !emitter.emit && emitterEnableTimer <= 0 ? emitterEnableTimer = 1e3 + 1e3 * emitter.maxLifetime : emitterEnableTimer > 0 && (emitterEnableTimer -= a, emitterEnableTimer <= 0 && (emitter.emit = !0)), particleCountDiv.innerHTML = emitter.particleCount + " 粒子")
        },
...
function() {
    var a = include("cloudkid.NodeWebkitApp"),
        ...
        c = (include("pixiparticles.Menu", !1), include("cloudkid.Browser"),
            function() {
                a.apply(this),
                    this.editor = new b({
                        framerate: "framerate",
